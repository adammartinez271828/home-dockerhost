...
- **db_recipes / web_recipes** — a 2-container [Tandoor Recipes](https://github.com/vabene1111/recipes) 2.x deployment (Postgres 16 + the Tandoor image, which since 2.0 bundles nginx + gunicorn and serves static and media itself on port 80). Caddy fronts it as `recipes.local` → `web_recipes:80`. The former `nginx_recipes` sidecar and `nginx_config` volume were removed with the 1.5.33 → 2.6.13 upgrade. Tandoor 2 DB migrations are **one-way**: run `make backup-db` before any Tandoor bump; rollback is the previous image plus `scripts/db-restore.sh --live <that dump>`.
...
- **kinboard** — [Kinboard](https://github.com/svenger87/kinboard) family dashboard, the one service **not** in this compose file. It is its own compose project (Supabase-style stack: `kinboard-db` Postgres 15, `kinboard-kong` gateway, `kinboard-webapp` Next.js, plus auth/rest/storage/realtime/imgproxy/cron) driven by upstream's `start.sh` from the git submodule `kinboard/upstream`, pinned to the tag matching `KINBOARD_TAG` (1.10.0). `kinboard/docker-compose.override.yml` (copied into the submodule) joins `kong` and `webapp` to `network_landockernet`, and mounts the one local migration `kinboard/migration_zzzzz_local_meal_plans_realtime.sql` into the webapp, which drops `meal_plans` from the realtime publication on every start to break an upstream write-storm loop (the entrypoint re-applies upstream migrations, which re-add it, on every boot, so a one-off `ALTER` does not stick; see "Local override: meal_plans realtime" in `kinboard/README.md`); its host ports (5432, 8100, 8543, 3001) are bound to **127.0.0.1 only**, the accepted exception to "only Caddy publishes a port". Caddy routes `kinboard.local` as one origin split by path: `/rest /auth /storage /realtime` → `kinboard-kong:8000`, everything else → `kinboard-webapp:3000`. Data in `~/kinboard-data/` (outside the repo); host `.env` lives inside the submodule (gitignored there; documented by `env.d/kinboard.env.example`). The nightly backup also dumps it (`db/kinboard/`, plus `storage-kinboard/`). `make kinboard-up/-status/-logs`; details and restore in `kinboard/README.md`. After a `git pull` that moves the submodule: `git submodule update --init`.

- **kitchen-kiosk** — the wall dashboard host, a second Pi (Pi 4, Wi-Fi, SD card) that shows `kinboard.local` in a Chromium kiosk. It runs no containers and has **no clone of this repo**; it is hand-provisioned, and `kiosk/README.md` is the build record (Imager settings, base config, packages, gotchas). Reach it with `ssh kiosk@kitchen-kiosk.local` (key, passwordless sudo). The compositor is `cage@tty1` running the tracked `kiosk/kinboard-kiosk` wrapper; `kiosk/install.sh` (`make kiosk-install`) deploys the wrapper, unit and PAM file over ssh, and `/etc/default/kinboard-kiosk` on the kiosk holds the rotation/scale/output knobs plus the nightly screen-off window (`KIOSK_SCREEN_OFF`/`ON`, 23:00 to 06:00 local), applied every minute by `kinboard-kiosk-screen.timer` via the tracked `kiosk/kinboard-kiosk-screen` script (`make kiosk-screen`). Only the display sleeps; a Pi 4 cannot suspend. A second tracked script, `kiosk/kinboard-kiosk-net` (root, minutely `kinboard-kiosk-net.timer`, knobs `KIOSK_NET_*`), is the **Wi-Fi reachability watchdog** added after the 2026-09-19 silent link death: it pings the gateway bound to `wlan0`, and after 5 failed minutes writes a forensic capture to `/var/log/kinboard-kiosk-net.log` (the journal is volatile) and climbs `wpa_cli reassociate` → `nmcli down/up` → reboot after 3 failed cycles; `make kiosk-net` / `make kiosk-net-log`. Chromium never reloads a crashed tab, so `kinboard-kiosk-crash.path` watches Crashpad's `Crash Reports/pending/` and `kiosk/kinboard-kiosk-crash` restarts `cage@tty1` within seconds of a dump (max `KIOSK_CRASH_MAX_PER_HOUR`, logged to `/var/log/kinboard-kiosk-crash.log`; `make kiosk-crash-log`); `kinboard-kiosk-restart.timer` restarts it nightly at 03:30 (inside the screen-off window) as a backstop, and the apt `DPkg::Post-Invoke` hook `kiosk/kinboard-kiosk-stale-chromium` restarts it after any upgrade that replaced the running Chromium binary. Its old `dockerhost` SSD is the shelved cold rollback: never reattach it.
...
make kiosk-install # deploy kiosk/ to the kitchen-kiosk Pi over ssh (R=1 restarts); kiosk-status/-logs/-restart/-screen/-net/-net-log/-crash-log too
