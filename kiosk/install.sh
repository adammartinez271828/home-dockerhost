...
# wrapper, unit and PAM file; /etc/default/kinboard-kiosk is created only if
...
for f in kinboard-kiosk cage@.service pam.d-cage kinboard-kiosk.defaults.example; do
...
scp -q "${ssh_opts[@]}" "$here/kinboard-kiosk" "$here/cage@.service" \
	"$here/pam.d-cage" "$here/kinboard-kiosk.defaults.example" "$KIOSK_HOST:$tmp/"
